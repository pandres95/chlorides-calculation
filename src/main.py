...
from chloride_model import fit_chloride_profile, calculate_x_alpha, interp_cross
from plotting import plot_profile
from excel_export import export_group_excel
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir", default="output", help="Directory for output files"
    )
    parser.add_argument(
        "--Ci",
        type=float,
        default=None,
        help="Global initial concentration override. If not set, uses defaults based on Cement Type.",
    )
    ...
    # CEM III: 0.00950%
    # CEM V: 0.00975%
    CI_DEFAULTS = {"CEM I": 0.01370, "CEM III": 0.00950, "CEM V": 0.00975}

    # Load data
    df = None
    try:
        # Try standard CSV first (comma separator, dot decimal)
        df = pd.read_csv(args.input_file)

        # If successfully read but looks like wrong separator (single column), force fallback
        if len(df.columns) <= 1:
            raise ValueError("Likely wrong separator (single column detected)")

    except Exception:
        # Fallback: Try European format (semicolon separator, comma decimal)
        try:
            df = pd.read_csv(args.input_file, sep=";", decimal=",")
    ...
    for name, group in df.groupby(group_cols):
        ...
        Cs, Dnss, Cs_std, Dnss_std, r_squared = fit_chloride_profile(
            fit_depths_m, fit_chlorides, t_seconds, Ci=Ci
        )
